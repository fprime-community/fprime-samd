...
// with the per-slot Fw::Buffer replaced by Samd21::ThinBuffer. See the header
// for rationale.
//
// ======================================================================
...
void ThinBufferManagerComponentImpl ::cleanup() {
    ...
    if (not this->m_cleaned) {
        // ThinBuffer is trivially destructible; no per-entry destructor calls needed
        // (unlike Fw::Buffer, which the fprime BufferManager destructs explicitly here).
        this->m_cleaned = true;
        // release memory
...
Fw::Buffer ThinBufferManagerComponentImpl ::bufferGetCallee_handler(const FwIndexType portNum,
    ...
    for (U16 buff = 0; buff < this->m_numStructs; buff++) {
        if ((not this->m_buffers[buff].allocated) and (size <= this->m_buffers[buff].size)) {
            ...
            // Build the caller-facing Fw::Buffer fresh from the stored ThinBuffer. This
            // temporary lives on the caller's stack only -- the persistent slot never
            // stores an Fw::Buffer.
            Fw::Buffer copy = this->m_buffers[buff].buff.getBuffer();
...
void ThinBufferManagerComponentImpl::setup(U16 mgrId,                    //!< manager ID
    ...
    for (U16 bin = 0; bin < ThinBufferManagerConfig::THINBUFFERMGR_MAX_NUM_BINS; bin++) {
    ...
    for (U16 bin = 0; bin < ThinBufferManagerConfig::THINBUFFERMGR_MAX_NUM_BINS; bin++) {
        if (this->m_bufferBins.bins[bin].numBuffers) {
            for (U16 binEntry = 0; binEntry < this->m_bufferBins.bins[bin].numBuffers; binEntry++) {
                ...
                // ThinBuffer is trivial, so a plain assignment (no placement new required) is
                // sufficient -- unlike the fprime BufferManager, which must placement-new an
                // Fw::Buffer here because it owns a non-trivial Fw::ExternalSerializeBuffer member.
                // context must be stored per-slot: bufferSendIn_handler reads it back (via
                // Fw::Buffer::getContext(), populated from ThinBuffer::getBuffer()) to identify
                // which slot a returned buffer belongs to.
                this->m_buffers[currStruct].buff =
                    Samd21::ThinBuffer(bufferMem, static_cast<U32>(this->m_bufferBins.bins[bin].bufferSize), context);
