...
// Configuration enums for ADC peripheral
...
class AdcDriver final : public AdcDriverComponentBase {
  ...
  private:
    ...
    //! Handler implementation for schedIn - checks for completed conversions and delivers results
    //! \param portNum The port number
    ...
    // Async conversion state (shared between readAdc, schedIn, and ISR)
    volatile bool m_conversionPending;   //!< Is a conversion in progress?
    volatile bool m_conversionComplete;  //!< Set by ISR when conversion done
    volatile bool m_overrunOccurred;     //!< Set by ISR if overrun error
